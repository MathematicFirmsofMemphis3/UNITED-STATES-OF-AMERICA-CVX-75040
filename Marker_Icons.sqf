/*
Author: Aaren
Can see map markers in 3D (this script should work in any mod)
return: none
*/
...
draw_marker = {
  {
		...
		if (_markerColorRaw == "ColorWEST") then {
				_markerColor = _ColorWEST;
		};
		if (_markerColorRaw == "ColorEAST") then {
		  _markerColor = _ColorEAST;
		};
		if (_markerColorRaw == "ColorGUER") then {
		  _markerColor = _ColorGUER;
		};
		if (_markerColorRaw == "ColorCIV") then {
...
handler_draw3d = addMissionEventHandler ["Draw3D",{
  call draw_marker;
}];
